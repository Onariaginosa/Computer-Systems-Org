...
void show_or_hide(mcw_game* game, int setter) {
    for (int y = 0;y < game->height; y++) {
        for (int x = 0; x < game->width; x++) {
            game->status[y][x] = setter;
        }
    }
}
...
void reveal_game_square(mcw_game* game, int x, int y) {
    if( is_in_game_bounds(game, x, y) && game->status[y][x] == hidden) {
        ...
        if(game->field[y][x] == open) {
            for (int i = -1; i < 2; i++){
                for(int j = -1; j <2; j++) {
                    if(is_in_game_bounds(game, x+i, y+j) && get_adjacent_weeper_count(game, x, y) == 0) {
...
bool is_game_over_win(mcw_game* game) {
    ...
    for( int y = 0; y < game->height; y++) {
        for(int x = 0; x < game->width; x++) {
            ...
            if(game->field[y][x] == open && game->status[y][x] == revealed) {
                hidden_safe++;
            }
            if(game->field[y][x] == open && game->status[y][x] == flagged) {
                wrong_bomb++;
            }
        }
    }
    if((hidden_weeper == 0 || hidden_safe == 0) && wrong_bomb == 0) {
        return true;
    } else {
        return false;
    }
